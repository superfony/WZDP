package com.epsmart.wzcc.http.request;
...
import com.epsmart.wzcc.db.table.TableImpi;
import com.epsmart.wzcc.db.table.TemplateTable;
import com.epsmart.wzcc.http.request.model.RequestParameter;
import com.epsmart.wzcc.http.response.model.Response;
import com.epsmart.wzcc.util.JavaUtil;
...
//			httpClient = new DefaultHttpClient();
    ...
    public void run() {
        try {
            ...
            Log.d("Exception", "Exception 服务器提交异常信息=" + e);
            HttpException exception = new HttpException(
...
//		}
        if (params.methodName.equals(RequestParamConfig.receiveList)) {
            ...
            request.addProperty("pageNo", Integer.toString(params.pageNo));
            request.addProperty("pageSize", Integer.toString(params.pageSize));
        } else if (params.methodName.equals(RequestParamConfig.getHouseDetail)) {
            request.addProperty("param", params.param);
        } else if (params.methodName.equals(RequestParamConfig.pullCommit)) {
            request.addProperty("userid", params.userid);
            request.addProperty("param", params.param);
            request.addProperty("type", params.user_type);
        } else if (params.methodName.equals(RequestParamConfig.warehouseInfo)) {
            request.addProperty("userid", params.userid);
        } else if (params.methodName.equals(RequestParamConfig.warehouseList)) {
            request.addProperty("username", params.userid);
            request.addProperty("pageNo", Integer.toString(params.pageNo));
            request.addProperty("pageSize", Integer.toString(params.pageSize));
        } else if (params.methodName.equals(RequestParamConfig.getWarehouseDetail)) {
            Log.w("head", ".........warehouseInfo...");
            request.addProperty("param", params.param);
        } else if (params.methodName.equals(RequestParamConfig.pullwarehourseSubmit)) {
        ...
        Log.w("AsyncHttp", "params=" + params.methodName + "......=" + RequestParamConfig.serviceNameSpace);
        ...
        System.out.println("Asy......back result=" + respStr);
        return respStr;
    }
    ...
    protected String getResultFroThrift() {
        ...
        Log.w("AsyncHttpRequest", "result=" + result);
        return result;
    }
