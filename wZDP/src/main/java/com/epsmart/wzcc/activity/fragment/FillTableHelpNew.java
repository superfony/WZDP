...
import com.epsmart.wzcc.util.DateTimePickerDialogUtil;
import com.epsmart.wzcc.widget.time.JudgeDate;
import com.epsmart.wzcc.widget.time.ScreenInfo;
import com.epsmart.wzcc.widget.time.WheelMain;
...
public class FillTableHelpNew {
		...
		for (int i = 0; i < size; i++) {
			...
			if (field.fieldView.equals("1")) {
				...
				image.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						((com.epsmart.wzcc.activity.CommonActivity) activity)
	...
	public String getparams() {
		...
		for (int i = 0; i < keyList.size(); i++) {
			strb.append("<param>").append("<key>").append(keyList.get(i))
					...
					.append(editList.get(i).getText().toString())
	...
	public void fillTableNew(ArrayList<BasicEntity> entityList) {
		...
		for (int i = 0; i < size; i++) {
			...
			/* 处理标题的显示 */
			be = entityList.get(i);
			String title = be.fields.get("title").fieldContent;
	...
	public Persion[] getArrayValues(HashMap<String, ArrayList<Persion>> map) {
		...
		if (groupName != null) {
			ArrayList<Persion> persionList = map
					.get(groupName);
...
//				else if (field.fieldChName.contains("供应商编码")&&null!=field.fieldContent) {
//					int info = Integer.valueOf(field.fieldContent);
//					String newinfo = "" + info;
//					editText.setText(newinfo);
//				}
...
//			else if (!TextUtils.isEmpty(field.fieldContent)
//					&& field.fieldView.equals("2")) {// 非空可修改
//				hslay = new HorizontalScrollView(activity);
//				hslp = new HorizontalScrollView.LayoutParams(linlayWv, linlayH);
//				hslay.setHorizontalScrollBarEnabled(false);
//				linlay = new LinearLayout(activity);
